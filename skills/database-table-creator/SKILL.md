...
```text
Changed:
- <migration path> — table/index/trigger summary
- <Kotlin path> — table/entity/repository/factory summary
- <test path> — coverage summary
Verified:
- <migration/test commands or "not run">
Risk:
- <rollback/backfill/deploy concern or "low">
```

If tests or migration commands were skipped, state the blocker and the exact
command the next agent should run.

## Auto-Clarity

Use explicit ordered prose for:

- migration order and rollback expectations
- backfills or changes to existing rows
- nullable-to-required transitions
- unique indexes on existing data
- any operation that could lock a large table

Do not compress these into fragments; execution order must be unambiguous.

## Boundaries

- Do not edit already-deployed migration files.
- Do not fill migration-number gaps without checking current sequence.
- Do not add foreign key constraints or hard deletes.
- Do not run migrations against shared or production databases.
- Do not create repository code without soft-delete filters and tests.
