...
```text
Checked:
- <route> — desktop/mobile — pass|fail
- <route> — desktop/mobile — pass|fail
Findings:
- <path/route>: <bug/risk>. <repro or screenshot path>.
Verified:
- <command or manual browser session>
Risk:
- <untested route/state or "low">
```

If no issues are found, write `Findings: none` and still list the checked
routes/viewports.

## Auto-Clarity

Use explicit prose when a bug involves:

- auth redirect loops
- payment, subscription, or entitlement UI
- destructive user actions
- data loss, hidden data, or wrong user data shown
- flaky failures that need a reproducible sequence

## Boundaries

- Do not change product code during QA unless the user asks for fixes.
- Do not treat warnings as bugs unless they break user behavior.
- Do not claim full coverage if login, permissions, seed data, or environment
  setup blocked a route.
- Do not leave a browser/server process running after the QA pass.
